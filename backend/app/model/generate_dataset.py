...
# Exact question bank from questions.py
...
SCORE_LABELS = {
    "weak":      {"range": (0,  40),  "description": "Very poor — missing structure, vague, too short, no examples"},
    "below_avg": {"range": (41, 55),  "description": "Below average — some content but missing key parts"},
    "average":   {"range": (56, 69),  "description": "Average — has some structure but lacks concrete examples"},
    "good":      {"range": (70, 84),  "description": "Good — clear structure, some examples, mostly complete"},
    "excellent": {"range": (85, 100), "description": "Excellent — full structure, concrete examples, measurable results"},
}

def generate_answers(question, field, category, difficulty, label, description, count=3):
    prompt = f"""Generate {count} different interview answers for this question.
...
Rules:
- Each answer must clearly match the quality level
- Make them realistic like real students would answer
- Vary length and style between answers
- Weak: vague, short, no structure, filler words like um/basically
- Excellent: specific, structured, concrete field-specific examples, measurable outcomes
...
["answer 1", "answer 2", "answer 3"]"""
    ...
    try:
        ...
        if "rate_limit_exceeded" in str(e):
            ...
            time.sleep(60)
            return generate_answers(question, field, category, difficulty, label, description, count)
        print(f"  Error: {e}")
        return []

def assign_score(label):
    import random
    low, high = SCORE_LABELS[label]["range"]
    return random.randint(low, high)

def detect_question_type(question, category):
    q = question.lower()
    if any(p in q for p in ["tell me about yourself", "introduce yourself"]):
        return "intro"
    if any(p in q for p in ["strength", "weakness"]):
        return "strength"
    if category == "technical":
        return "technical"
    return "star"

def generate_dataset(output_path="app/model/dataset.json"):
    dataset = []
    ...
    total_questions = sum(
        ...
        for difficulties in categories.values()
        for questions in difficulties.values()
    )

    print(f"Total questions in bank: {total_questions}")
    print(f"Generating 3 answers x 5 quality levels = 15 samples per question")
    print(f"Expected dataset size: ~{total_questions * 15} samples\n")

    for field, categories in QUESTION_BANK.items():
        print(f"\n📚 Field: {field}")
            ...
            for difficulty, questions in difficulties.items():
                for question in questions:
                    ...
                    if question in completed_questions:
                        print(f"  ⏭ Skipping (already done): {question[:50]}...")
                        continue

                    q_type = detect_question_type(question, category)
                    print(f"  [{category}/{difficulty}] {question[:50]}...")

                    for label, info in SCORE_LABELS.items():
                        answers = generate_answers(
                            question, field, category,
                            difficulty, label, info["description"],
                            count=3
                        )
                    ...
                    print(f"    ✓ {len(dataset)} total samples saved")
