...
const DIFFICULTIES = [
  ...
  { d: 'advanced', color: '#f97316', label: '✨ AI' },
];
...
export default function Home({ user }) {
  ...
  const isCustom = !FIELDS.map(f => f.name).includes(field);
  ...
  const handleAudioAnalyze = async () => {
    if (!question.trim()) { setError('Please select a question first.'); return; }
    if (!audioBlob) { setError('Please record your answer first.'); return; }
    setLoading(true); setError(''); setResult(null);
    try {
      const fd = new FormData();
      fd.append('question', question);
      fd.append('audio', audioBlob, 'answer.webm');
      fd.append('use_groq', isAiGenerated ? 'true' : 'false');
      const res = await fetch('http://127.0.0.1:8000/api/analyze/audio', { method: 'POST', body: fd });
      if (!res.ok) throw new Error('Server error.');
      const data = await res.json();
      if (data.transcript) setTranscript(data.transcript);
      setResult(data);
  ...
  const currentField = FIELDS.find(f => f.name === field);
    ...
    <div style={{ minHeight: '100vh' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        <div style={{
          ...
          <div>
            <h2 style={{ fontSize: '22px', fontWeight: 'bold', marginBottom: '4px' }}>
              Welcome back, <span style={{ color: orange }}>{user?.displayName?.split(' ')[0]}</span> 👋
            </h2>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '14px' }}>
            {FIELDS.map(f => (
              <button key={f.name} onClick={() => { setField(f.name); setCustomField(''); setResult(null); }} style={{
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[
                ...
                <button key={cat.id} onClick={() => setCategory(cat.id)} style={{
                  ...
                  boxShadow: category === cat.id ? `0 0 14px rgba(249,115,22,0.35)` : 'none',
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {DIFFICULTIES.map(({ d, color, label }) => (
                <button key={d} onClick={() => setDifficulty(d)} style={{
                  flex: 1, padding: '10px 4px', borderRadius: '8px', fontSize: '12px',
                  fontWeight: difficulty === d ? 'bold' : 'normal',
                  background: difficulty === d ? color : '#1a1a1a',
        ...
        <div style={{
          ...
          <p style={{ color: '#888', fontSize: '13px', marginBottom: '14px', letterSpacing: '1px', textTransform: 'uppercase' }}>
            Select a Question
            {isAiGenerated && <span style={{ color: orange, marginLeft: '10px', fontSize: '12px' }}>✨ AI Generated</span>}
          </p>
          {loadingQ ? (
            <div style={{ color: '#555', textAlign: 'center', padding: '30px', fontSize: '14px' }}>
              {difficulty === 'advanced' || isCustom ? '✨ Generating with Groq AI...' : 'Loading questions...'}
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {questions.map((q, i) => (
                <div key={i} onClick={() => setQuestion(q)} style={{
                  padding: '12px 16px',
                  background: question === q ? 'rgba(249,115,22,0.1)' : '#0a0a0a',
                  border: `1px solid ${question === q ? orange : '#1a1a1a'}`,
                  borderRadius: '10px',
                  color: question === q ? orange : '#aaa',
                  fontSize: '14px', lineHeight: '1.5', cursor: 'pointer',
                  boxShadow: question === q ? `0 0 16px rgba(249,115,22,0.2)` : 'none',
                  transition: 'all 0.2s',
                  ...
                  <span style={{
                    ...
                    background: question === q ? orange : '#1a1a1a',
                    color: question === q ? '#000' : '#333',
        ...
        {question && (
          <div style={{
            ...
            <div>
              <p style={{ color: '#555', fontSize: '11px', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '1px' }}>
                Selected Question
              </p>
              <p style={{ color: orange, fontSize: '15px', lineHeight: '1.6' }}>{question}</p>
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          {[
            ...
            <button key={m.id} onClick={() => {
              setMode(m.id); setResult(null); setError('');
              window.speechSynthesis.cancel(); setSpeaking(false);
            }} style={{
              padding: '14px 10px',
              ...
              <div style={{ fontSize: '14px', fontWeight: 'bold' }}>{m.label}</div>
          ...
          <div>
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#333' }}>
                  <span style={{ color: answer.split(/\s+/).filter(Boolean).length >= 50 ? '#22c55e' : '#ef4444' }}>
                    {answer.trim().split(/\s+/).filter(Boolean).length} / 50 words min
                  </span>
                </div>
              </div>
              <textarea
                value={answer}
                onChange={e => setAnswer(e.target.value)}
                placeholder="Type your answer here using the STAR method:&#10;&#10;Situation → Task → Action → Result&#10;&#10;Aim for at least 50 words with a concrete example."
                rows={8}
                style={{
                  width: '100%', padding: '14px',
                  background: '#0a0a0a', color: '#fff',
                  border: `1px solid ${answer.length > 0 ? (answer.split(/\s+/).filter(Boolean).length >= 50 ? '#22c55e33' : '#f9731633') : '#222'}`,
          ...
          <div>
            <div style={{
              ...
              {question ? (
                <div style={{ marginBottom: '24px' }}>
                  <div style={{
                    background: 'rgba(249,115,22,0.06)', border: `1px solid ${orange}33`,
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button onClick={() => speakQuestion(question)} disabled={speaking} style={{
                      ...glowBtn, flex: 1, padding: '12px', fontSize: '14px',
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
                {!recording ? (
                  <button onClick={startRecording} style={{ ...glowBtn, flex: 1, padding: '14px', fontSize: '15px' }}>
                    ● Start Recording
                  </button>
                ) : (
                  <button onClick={stopRecording} style={{
              ...
              {recording && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#ef4444', fontSize: '14px', marginBottom: '16px' }}>
              ...
              {audioURL && (
                <div style={{ marginBottom: '8px' }}>
                  <p style={{ color: '#555', fontSize: '13px', marginBottom: '8px' }}>Review your recording:</p>
                  <audio controls src={audioURL} style={{ width: '100%', borderRadius: '8px' }} />
                </div>
              )}
            </div>
        ...
        {mode === 'video' && (
          <div>
            <VideoRecorder
              question={question}
              isAiGenerated={isAiGenerated}
              onResult={(data) => { setResult(data); saveToHistory(data, question); }}
            />
          </div>
        )}
